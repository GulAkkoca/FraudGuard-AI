...
def analyze_review_risk(product: Product) -> AgentOutput:
    ...
    # 5. Rating dağılım analizi (U-eğrisi / kutuplaşma)
    r_counts = stats.rating_counts or {}
    c1 = r_counts.get("1", 0)
    c2 = r_counts.get("2", 0)
    c3 = r_counts.get("3", 0)
    c4 = r_counts.get("4", 0)
    c5 = r_counts.get("5", 0)
    total_ratings_from_counts = sum([c1, c2, c3, c4, c5])
    if total_ratings_from_counts >= 15:
        middle_ratings = c2 + c3 + c4
        extreme_ratings = c1 + c5
        if extreme_ratings / total_ratings_from_counts > 0.85 and middle_ratings / total_ratings_from_counts < 0.15 and c1 > 0 and c5 > 0:
            risk += 10
            reason_codes.append("BIMODAL_RATING_DISTRIBUTION")
            evidence.append("Değerlendirme dağılımında kutuplaşma (U-eğrisi) var: Yorumlar neredeyse sadece 5 ve 1 yıldızlarda yığılmış.")

    # 6. Yorum / Değerlendirme oranı analizi (Yorum silinme / engellenme şüphesi)
    if stats.total_comment_count is not None and stats.total_rating_count is not None and stats.total_rating_count > 50:
        comment_to_rating_ratio = stats.total_comment_count / stats.total_rating_count
        if comment_to_rating_ratio < 0.05:
            risk += 5
            reason_codes.append("EXTREMELY_LOW_COMMENT_RATIO")
            evidence.append(f"Toplam değerlendirmeye ({stats.total_rating_count}) kıyasla yazılı yorum oranı ({stats.total_comment_count} yorum) şüpheli derecede düşük (%{comment_to_rating_ratio*100:.1f}). Olumsuz yorumların elenmiş veya silinmiş olma riski yüksek.")

    # 7. Etiket analizi (Olumsuz kullanıcı etiketleri)
    negative_tag_keywords = ["iade", "kırık", "kirik", "kalitesiz", "yanlış", "yanlis", "sahte", "defolu", "bozuk", "küçük", "kucuk", "dar", "kötü", "kotu"]
    found_negative_tags = [t for t in (stats.tags or []) if any(kw in t.lower() for kw in negative_tag_keywords)]
    if found_negative_tags:
        risk += 20
        reason_codes.append("NEGATIVE_USER_TAGS")
        evidence.append(f"Öne çıkan kullanıcı etiketlerinde olumsuz geri bildirimler tespit edildi: {', '.join(found_negative_tags)}")

    # 8. 5 yıldız oranı — tek başına değil, diğer sinyallerle birlikte değerlendir
